...
<br>
	...
	<table>
		...
		%>
		<tr>
			<td><a href=UserServlet?mode=read&id=<%=u.getIduser()%>>
					<%=u.getNome()%>
			</a></td>
			<td><%=u.getPassword()%></td>
			<td><%=u.getUsertype()%></td>
			<td><a href=UserServlet?mode=read&update=true&id=<%=u.getIduser()%>>Edit</a>
			</td>
			<td><a href=UserServlet?mode=delete&id=<%=u.getIduser()%>>Delete</a>
			</td>
